...
def render_markdown(voltage: dict[str, object], volume: dict[str, object], neb: dict[str, object] | None) -> str:
    lines = [
        ...
        f"- Voltage class: `{voltage['voltage_class']}`",
        ...
        f"- Host density (g/cm^3): `{voltage['host_density_g_cm3']:.4f}`" if voltage["host_density_g_cm3"] is not None else "- Host density (g/cm^3): `n/a`",
        ...
        f"- Specific energy (Wh/kg): `{voltage['specific_energy_Wh_kg']:.2f}`" if voltage["specific_energy_Wh_kg"] is not None else "- Specific energy (Wh/kg): `n/a`",
        f"- Volumetric energy (Wh/L): `{voltage['volumetric_energy_Wh_L']:.2f}`" if voltage["volumetric_energy_Wh_L"] is not None else "- Volumetric energy (Wh/L): `n/a`",
        "",
        "## Volume Change",
        ...
        f"- Breathing class: `{volume['breathing_class']}`",
    ...
    if neb is not None:
        lines.extend(
            [
                ...
                f"- Kinetic class: `{neb['kinetic_class']}`",
